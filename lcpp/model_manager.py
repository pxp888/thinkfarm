...
logger = logging.getLogger("thinkfarm.model_manager")

# ---------------------------------------------------------------------------
# Constants & Paths
# ---------------------------------------------------------------------------
_CONFIG_DIR = Path.home() / ".thinkfarm"
_CONFIG_PATH = _CONFIG_DIR / "custom.ini"
...
class ModelManager:
    ...
    def get_gpu_specs(self) -> tuple[int, float]:
        ...
        if gpu_name:
            ...
            if bandwidth:
                logger.info(f"{_PFX} Detected GPU Name: '{gpu_name}' -> Bandwidth: {bandwidth} GB/s")
            else:
                logger.info(f"{_PFX} Detected GPU Name: '{gpu_name}' (No mapped bandwidth)")
        ...
        if bandwidth is None:
            ...
            if vram_gb >= 24:
                ...
                bandwidth = 150.0
            logger.info(f"{_PFX} Fallback GPU Bandwidth heuristic: {bandwidth} GB/s (based on {vram_gb:.1f} GB VRAM)")
    ...
    def _load_manifest_names(self):
        ...
        if _MANIFEST_NAME_FILE.exists():
            try:
                ...
                logger.error(f"{_PFX} Error loading manifest names: {e}")
                self.manifest = set()
    ...
    def _save_manifest(self):
        ...
        try:
            ...
            logger.error(f"{_PFX} Error saving manifest names: {e}")
    ...
    def _load_user_models(self):
        ...
        if _USER_MODELS_FILE.exists():
            try:
                ...
                logger.error(f"{_PFX} Error loading user_models: {e}")
    ...
    def _save_user_models(self):
        ...
        try:
            ...
            logger.error(f"{_PFX} Error saving user_models: {e}")
    ...
    async def get_demand_chart(self) -> List[dict]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0, follow_redirects=True, verify=True) as client:
                ...
                logger.info(f"{_PFX} Fetching demand chart from: {url}")
                resp = await client.get(url, headers=headers)
                resp.raise_for_status()
                return resp.json().get("data", [])
        except httpx.HTTPStatusError as e:
            logger.error(f"{_PFX} HTTP Error fetching demand chart: {e.response.status_code} - {e.response.text}")
            return []
        except Exception as e:
            logger.error(f"{_PFX} Error fetching demand chart: {e}")
            return []
    ...
    async def get_remote_model_info(self, model_name: str) -> Optional[dict]:
        ...
        try:
            ...
            logger.error(f"{_PFX} HTTP Error fetching remote info for {model_name}: {e.response.status_code}")
            return None
        except Exception as e:
            logger.error(f"{_PFX} Error fetching remote info for {model_name}: {e}")
            return None
    ...
    async def optimize_portfolio(self, storage_limit_gb: float) -> tuple[List[str], str]:
        ...
        logger.info(f"{_PFX} Starting portfolio optimization (Limit: {storage_limit_gb} GB)")
        ...
        if new_user_models:
            ...
            logger.info(f"{_PFX} user_models updated ({len(self.user_models)} total): {sorted(self.user_models)}")
        ...
        if total_vram == 0:
            logger.warning(f"{_PFX} No GPU VRAM detected. Skipping portfolio optimization.")
            return ([], "")
        ...
        if _SLOPEMON_DATA_PATH.exists():
            try:
                ...
                logger.info(f"{_PFX} Loaded {len(slope_peer_thresholds)} network peak thresholds from slopemon.")
            except Exception as e:
                logger.error(f"{_PFX} Error reading slopemon file: {e}")
        ...
        if _CONFIG_PATH.exists():
            try:
                ...
                logger.error(f"{_PFX} Error reading min_acceptable_tps from config: {e}")
        logger.info(f"{_PFX} Default min_acceptable_tps: {min_acceptable_tps} TPS")
        ...
        for item in demand:
            ...
            if platform.system().lower() == "darwin":
                ...
                if not any(pat in model.lower() for pat in mac_patterns):
                    logger.info(f"{_PFX} Skipping {model}: not a Mac-specific model format.")
                    continue
            ...
            try:
                ...
                logger.info(f"{_PFX} Suitability check for {model}: fits={fits_in_vram}, tps={estimated_tps:.1f} (target threshold: {slope_peer_thresholds.get(model, min_acceptable_tps):.1f} TPS, VRAM={estimated_vram / (1024**3):.2f} GB)")
            except Exception as e:
                ...
                if fits_in_vram and estimated_vram > 0:
                    ...
                    estimated_tps = 0.0
                logger.warning(f"{_PFX} [Fallback] Calculated suitability failed ({e}). Estimated VRAM for {model}: {estimated_vram / (1024**3):.2f} GB, fits={fits_in_vram}, tps={estimated_tps:.1f}")
            except Exception as e:
                pass

            # Filter candidates based on VRAM fit
            if not fits_in_vram:
                logger.info(f"{_PFX} Skipping {model}: does not fit in VRAM.")
                continue
        ...
        if to_remove:
            if random.random() < 0.8:
                logger.info(f"{_PFX} Optimization cycle requires removals, but skipped due to 80% coin flip.")
                return ([], "")
        ...
        for model in to_remove:
            logger.info(f"{_PFX} Removing managed model: {model}")
        ...
        for model in to_add:
            ...
            logger.info(f"{_PFX} Pulling managed model: {model}")
            stream_ok = False
            try:
                ...
                logger.error(f"{_PFX} Pull stream error for {model}: {e}")
            ...
            if model in local_names:
                ...
                logger.info(f"{_PFX} Successfully pulled {model}")
            else:
                logger.warning(f"{_PFX} Pull finished but model not found on disk for {model}.")
