...
# Read from ~/.thinkfarm/custom.ini [provider] context_pressure, default 0.9.
def _get_context_pressure() -> float:
    """Load context_pressure from custom.ini, defaulting to 0.9."""
    try:
        config_dir = os.path.expanduser("~/.thinkfarm")
        config_path = os.path.join(config_dir, "custom.ini")
...
def _sync_recreate_custom_models(limits: Dict[str, int]) -> None:
    """llama.cpp does not support custom model creation; this is a no-op."""
    pass
...
async def run_context_probing(
    ...
    """
    Query the context size from GET /props on the running llama-server.
    Saves the context limits and baseline metrics to cache files.
    """
    ...
    try:
        ...
        print(f"{_PFX} Failed to fetch n_ctx from llama-server `/props`: {e}. Using default 4096.")
    ...
    try:
        ...
        print(f"{_PFX} Failed to save performance baselines: {e}")
...
async def _probe_new_models(
    ...
    for model_name in model_names:
        ...
        # Get full info to check eligibility and type
        upper_bound, is_embed, is_eligible = await _get_model_info(base_url, model_name)
        if not is_eligible:
