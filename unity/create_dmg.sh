...
DETECTED_APP=$(find "${SCRIPT_DIR}/dist" -maxdepth 1 -name "thinkfarm*.app" 2>/dev/null | head -n 1)
DEFAULT_APP=$(basename "${DETECTED_APP:-thinkfarm.app}")
...
hdiutil create -volname "thinkfarm8" -srcfolder "${STAGING_DIR}" -ov -format UDZO "${DMG_PATH}"
...
echo "   spctl --assess --type open --verbose ${DMG_PATH}"
